...
class OffloadTab {
	...
	public static function render( array $settings ): void {
		...
		<div class="cloudflare-r2-offload-cdn-tab-content" id="tab-offload">
			<h2><?php esc_html_e( 'Offload Settings', 'cloudflare-r2-offload-cdn' ); ?></h2>
			<p class="description"><?php esc_html_e( 'Configure how media files are offloaded to R2 storage.', 'cloudflare-r2-offload-cdn' ); ?></p>

			<table class="form-table">
				<tr>
					<th scope="row">
						<label for="auto_offload"><?php esc_html_e( 'Auto-Offload on Upload', 'cloudflare-r2-offload-cdn' ); ?></label>
					</th>
					<td>
						...
						<p class="description"><?php esc_html_e( 'Automatically offload media files to R2 when uploaded.', 'cloudflare-r2-offload-cdn' ); ?></p>
					</td>
				</tr>
				<tr>
					<th scope="row">
						<label for="batch_size"><?php esc_html_e( 'Batch Size', 'cloudflare-r2-offload-cdn' ); ?></label>
					</th>
					<td>
						<select id="batch_size" name="batch_size" class="regular-text">
							<option value="10" <?php selected( 10, $settings['batch_size'] ?? 25 ); ?>>
								<?php esc_html_e( '10 files per batch', 'cloudflare-r2-offload-cdn' ); ?>
							</option>
							<option value="25" <?php selected( 25, $settings['batch_size'] ?? 25 ); ?>>
								<?php esc_html_e( '25 files per batch', 'cloudflare-r2-offload-cdn' ); ?>
							</option>
							<option value="50" <?php selected( 50, $settings['batch_size'] ?? 25 ); ?>>
								<?php esc_html_e( '50 files per batch', 'cloudflare-r2-offload-cdn' ); ?>
							</option>
						</select>
						<p class="description"><?php esc_html_e( 'Number of files to process in each batch during bulk offload.', 'cloudflare-r2-offload-cdn' ); ?></p>
					</td>
				</tr>
				<tr>
					<th scope="row">
