...
class SystemInfoTab {
	...
	public static function render(): void {
		...
		<div class="cloudflare-r2-offload-cdn-tab-content" id="tab-system-info">
			<h2><?php esc_html_e( 'System Information', 'cloudflare-r2-offload-cdn' ); ?></h2>
			<p class="description"><?php esc_html_e( 'System status and debug information for troubleshooting.', 'cloudflare-r2-offload-cdn' ); ?></p>
	...
	private static function render_system_status( array $settings ): void {
		...
		<div class="settings-section cfr2-system-status">
			<h3><?php esc_html_e( 'System Status', 'cloudflare-r2-offload-cdn' ); ?></h3>
	...
	private static function get_system_statuses( array $settings ): array {
		...
		$statuses[] = array(
			...
			'value'  => CLOUDFLARE_R2_OFFLOAD_CDN_VERSION,
			'status' => 'info',
		);
		...
		$statuses[] = array(
			'label'  => __( 'WordPress Version', 'cloudflare-r2-offload-cdn' ),
			'value'  => $wp_version,
			'status' => $wp_status,
		);
		...
		$statuses[]    = array(
			'label'  => __( 'R2 Credentials', 'cloudflare-r2-offload-cdn' ),
			'value'  => $r2_configured ? __( 'Configured', 'cloudflare-r2-offload-cdn' ) : __( 'Not configured', 'cloudflare-r2-offload-cdn' ),
		...
		$statuses[]    = array(
			'label'  => __( 'Cloudflare API Token', 'cloudflare-r2-offload-cdn' ),
			'value'  => $cf_configured ? __( 'Configured', 'cloudflare-r2-offload-cdn' ) : __( 'Not configured', 'cloudflare-r2-offload-cdn' ),
		...
		$statuses[]      = array(
			'label'  => __( 'Worker Status', 'cloudflare-r2-offload-cdn' ),
			'value'  => $worker_deployed ? __( 'Deployed', 'cloudflare-r2-offload-cdn' ) : __( 'Not deployed', 'cloudflare-r2-offload-cdn' ),
		...
		$statuses[]  = array(
			'label'  => __( 'CDN Status', 'cloudflare-r2-offload-cdn' ),
			'value'  => $cdn_enabled ? __( 'Enabled', 'cloudflare-r2-offload-cdn' ) : __( 'Disabled', 'cloudflare-r2-offload-cdn' ),
		...
		$statuses[]   = array(
			'label'  => __( 'cURL Extension', 'cloudflare-r2-offload-cdn' ),
			'value'  => $curl_enabled ? __( 'Enabled', 'cloudflare-r2-offload-cdn' ) : __( 'Not available', 'cloudflare-r2-offload-cdn' ),
		...
		$statuses[]      = array(
			'label'  => __( 'OpenSSL Extension', 'cloudflare-r2-offload-cdn' ),
			'value'  => $openssl_enabled ? __( 'Enabled', 'cloudflare-r2-offload-cdn' ) : __( 'Not available', 'cloudflare-r2-offload-cdn' ),
		...
		$statuses[]    = array(
			'label'  => __( 'Max Execution Time', 'cloudflare-r2-offload-cdn' ),
			'value'  => 0 === (int) $max_execution ? __( 'Unlimited', 'cloudflare-r2-offload-cdn' ) : $max_execution . 's',
			'status' => $exec_status,
		);
	...
	private static function render_debug_info( array $settings ): void {
		...
		<div class="settings-section cfr2-debug-info">
			<h3><?php esc_html_e( 'Debug Information', 'cloudflare-r2-offload-cdn' ); ?></h3>
			<p class="description"><?php esc_html_e( 'Copy this information when requesting support.', 'cloudflare-r2-offload-cdn' ); ?></p>
			...
			<p>
				<button type="button" class="button" onclick="navigator.clipboard.writeText(document.getElementById('cfr2-debug-textarea').value); this.textContent='Copied!';">
					<?php esc_html_e( 'Copy to Clipboard', 'cloudflare-r2-offload-cdn' ); ?>
				</button>
			</p>
		</div>
		<?php
	...
	private static function get_debug_info( array $settings ): string {
		...
		// Plugin Info.
		$lines[] = '## Plugin';
		$lines[] = 'Version: ' . CLOUDFLARE_R2_OFFLOAD_CDN_VERSION;
		$lines[] = '';
