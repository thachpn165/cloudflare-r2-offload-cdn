...
class QueueProcessor {
	...
	public static function process(): void {
		...
		foreach ( $items as $item ) {
			...
			BulkOperationLogger::log(
				...
				$result['message'] ?? ( $result['success'] ? __( 'Offloaded successfully', 'cloudflare-r2-offload-cdn' ) : __( 'Operation failed', 'cloudflare-r2-offload-cdn' ) )
			);
	...
	private static function delete_from_r2( R2Client $r2, int $attachment_id ): array {
		...
		if ( ! $r2_key ) {
			return array(
				'success' => false,
				'message' => __( 'No R2 key found', 'cloudflare-r2-offload-cdn' ),
			);
		}
