...
class WorkerDeployer {
	...
	public function deploy( array $config ): array {
		...
		if ( ! $verify['success'] ) {
			return array(
				'success' => false,
				'message' => __( 'Invalid API token', 'cloudflare-r2-offload-cdn' ),
				'steps'   => $steps,
			);
		}
		$steps[] = array(
	...
	private function get_worker_script( array $config ): string {
		$template_path = CLOUDFLARE_R2_OFFLOAD_CDN_PATH . 'src/Templates/worker-script.js';
		$script        = file_get_contents( $template_path );

		// Replace placeholders.
		$script = str_replace( '{{VERSION}}', CLOUDFLARE_R2_OFFLOAD_CDN_VERSION, $script );
	...
	private function configure_route( string $domain ): array {
		...
		if ( ! $zone_id ) {
			return array(
				...
				'message' => sprintf(
					...
					__( 'Zone not found for domain: %s', 'cloudflare-r2-offload-cdn' ),
					$base_domain
				),
			);
		}
		...
		return array(
			'success' => false,
			'message' => $result['errors'][0]['message'] ?? __( 'Failed to configure route', 'cloudflare-r2-offload-cdn' ),
		);
	}
}
